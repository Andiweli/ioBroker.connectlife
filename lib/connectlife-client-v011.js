...
class ConnectLifeClient extends BaseConnectLifeClient {
    constructor(options) {
        ...
        this.scheduleTimeout = typeof options.scheduleTimeout === "function" ? options.scheduleTimeout : null;
    }
    ...
    canUseExistingAccessToken() {
        return Boolean(this.accessToken && Date.now() < this.accessTokenHardValidUntil - TOKEN_EXPIRY_SAFETY_MS);
    }
    ...
    wait(milliseconds) {
        return new Promise(resolve => {
            if (this.scheduleTimeout) {
                this.scheduleTimeout(resolve, milliseconds);
                return;
            }
