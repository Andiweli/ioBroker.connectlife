...
class ConnectLifeClient extends BaseConnectLifeClient {
    ...
    async obtainToken(force) {
        ...
        if (now < this.loginBlockedUntil) {
            ...
            await this.wait(this.loginBlockedUntil - now);
            this.loginBlockedUntil = 0;
        }
        ...
        if (this.canRefreshAccessToken()) {
            try {
                ...
                if (this.isRateLimitError(error)) {
                    return this.handleRateLimit(error, force);
                }
    ...
    async handleRateLimit(error, force) {
        ...
        this.log.warn(
            `ConnectLife login is temporarily rate-limited. ` +
                `The adapter will wait silently and retry in ${minutes} minute(s).`,
        );
        await this.wait(delay);
        this.loginBlockedUntil = 0;
        return this.obtainToken(force);
    }
